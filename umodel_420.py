...
"""
Version': '4.2.0' edited by floxay
...
def pskimport(filepath,
    ...
    if Normals is not None:
        ...
        # 使用shade_auto_smooth操作符设置自动平滑
        bpy.ops.object.shade_auto_smooth(use_auto_smooth=True, angle=0.523599)  # 角度设置为30度（0.523599弧度）
    ...
    if bImportbone:
    # 使用普通的Python列表来保存骨骼名称
        bone_group_unused = []
        bone_group_nochild = []

        for psk_bone in psk_bones:
            pose_bone = armature_obj.pose.bones.get(psk_bone.name)

            if pose_bone is not None:
                if psk_bone.have_weight_data:
                    if len(psk_bone.children) == 0:
                        # 向Python列表中添加骨骼名称
                        bone_group_nochild.append(pose_bone.name)
                else:
                    bone_group_unused.append(pose_bone.name)

        # 可以将列表保存为自定义属性，或者做其他处理
        armature_obj['bone_group_unused'] = bone_group_unused
        armature_obj['bone_group_nochild'] = bone_group_nochild

        # 打印分组信息，供调试
        print("Unused bones:", bone_group_unused)
        print("No children bones:", bone_group_nochild)
